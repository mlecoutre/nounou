function RegistrationCtrl($scope, User, Nurse, Children, AppService) {
  ...
  $scope.user = new User({accountId: $scope.token.accountId});
  $scope.secondUserPassword;
  $scope.users = listUsers();

  $scope.nurses = listNurses();
  $scope.nurse =  new Nurse({accountId:  $scope.token.accountId});

  $scope.child =  new Children({accountId:  $scope.token.accountId});
  ...
  i18n.init(function(t) {
    ...
    $scope.addUser = function(){
      ...
      User.save($scope.user, function(usr) {
        ...
        $scope.user = new User();
        $scope.token.accountId = usr.accountId;
        listUsers();
   ...
   function listChildren() {
    return $scope.children  = Children.query({childId:0, filter: 'account', value:  $scope.token.accountId});
  };
